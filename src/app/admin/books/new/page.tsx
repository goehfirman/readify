...
export default function NewBook() {
  ...
  const handleSubmit = async (type: "draft" | "publish") => {
    // Basic Validation
    if (!title.trim() || !author.trim() || !level || !grade || !sdg) {
      alert("Mohon lengkapi data wajib (Judul, Pengarang, Level, Jenjang, dan SDG) sebelum menyimpan.");
      return;
    }
    ...
    try {
      await addBook(newBook, (progress, message) => {
      ...
      console.error(err);
      alert("Gagal memperbarui buku. Silakan coba lagi nanti.");
      setIsSubmitting(false);
    }
  };
      ...
      <main className="flex-1 ml-72 min-h-screen relative overflow-hidden pb-20">
        ...
        <header className="px-10 py-8 flex justify-between items-center relative z-10 border-b-4 border-white">
           <div className="flex items-center gap-4">
              ...
              <div>
                 <h1 className="text-3xl font-black uppercase tracking-tighter leading-none text-[#333333]">Tambah <span className="text-[#FFB347]">Buku Baru</span></h1>
                 <p className="text-sm font-bold text-[#A0AEC0] mt-1">Isi formulir untuk menambahkan buku ke perpustakaan digital</p>
              </div>
           </div>
        </header>
        ...
        <div className="p-10 relative z-10 max-w-5xl">
           ...
           <div className="bg-white rounded-[32px] p-8 md:p-10 border-4 border-[#E2E8F0] shadow-sm mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="md:col-span-2">
                    ...
                    <input type="text" placeholder="Nama penulis buku..." value={author} onChange={(e) => setAuthor(e.target.value)} className={inputClass} />
                 </div>
                 ...
                 <div className="md:col-span-2">
                    ...
                    <textarea 
                       placeholder="Jelaskan singkat mengenai isi buku agar pengguna tertarik membaca..." 
                       value={synopsis} 
           ...
           <div className="bg-white rounded-[32px] p-8 md:p-10 border-4 border-[#E2E8F0] shadow-sm mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 {/* Upload Cover */}
                 <div>
                    <label className={labelClass}>Unggah Gambar Sampul (JPG/PNG)</label>
                    <input type="file" accept="image/jpeg,image/png" ref={coverInputRef} onChange={handleCoverUpload} className="hidden" />
                    <button 
                    ...
                    >
                       ...
                       <span className="text-xs font-black uppercase tracking-widest">Klik untuk Unggah</span>
                       <span className="text-[10px] font-bold">JPG, PNG (Maks. 5 MB)</span>
                    </button>
                    
                    {/* AI Generate Cover */}
                    <div className="mt-6">
                       <label className={labelClass}>Atau Generate dengan AI ✨</label>
                       <div className="flex gap-3">
                          <input 
                             type="text" 
                             placeholder="Deskripsikan gambar sampul yang diinginkan..." 
                             value={aiPrompt} 
                          ...
                          <button 
                             ...
                             className={`px-6 py-3 rounded-2xl text-sm font-black uppercase tracking-wider border-4 transition-all flex items-center gap-2 shrink-0 ${isGenerating ? 'bg-[#F8FAFC] text-[#A0AEC0] border-[#E2E8F0] cursor-wait' : 'bg-[#FFB347] text-white border-[#E69A2E] shadow-[0_4px_0_#E69A2E] hover:shadow-[0_2px_0_#E69A2E] hover:translate-y-[2px] active:shadow-none active:translate-y-1'}`}
                          >
                             {isGenerating ? (
                                <>
                       ...
                       {coverPreview ? (
                          <>
                             ...
                             <button 
                                ...
                                className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white/90 border-2 border-[#E2E8F0] flex items-center justify-center text-[#FF4757] hover:bg-[#FFE2E5] transition-all shadow-sm z-10"
                             >
                          ...
                          <div className="flex flex-col items-center gap-2 text-[#A0AEC0]">
                             ...
                             <p className="text-xs font-black uppercase tracking-widest">Belum ada sampul</p>
                          </div>
                       )}
                    </div>
                 </div>
           ...
           <div className="bg-white rounded-[32px] p-8 md:p-10 border-4 border-[#E2E8F0] shadow-sm mb-8">
              <div className="flex items-center justify-between mb-8 border-b-4 border-[#F8FAFC] pb-4">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                       ...
                       <p className="text-[10px] font-black text-[#A0AEC0] uppercase tracking-widest">{pages.length} halaman</p>
                    </div>
                 </div>
                 <button 
                    onClick={addPage}
                    className="px-5 py-3 rounded-2xl text-sm font-black uppercase tracking-wider bg-[#FFB347] text-white border-4 border-[#E69A2E] shadow-[0_4px_0_#E69A2E] hover:shadow-[0_2px_0_#E69A2E] hover:translate-y-[2px] active:shadow-none active:translate-y-1 transition-all flex items-center gap-2"
                 >
              ...
              <div className="space-y-6">
                 {pages.map((page, index) => (
                    <div key={index} className="bg-[#F8FAFC] rounded-2xl p-6 border-2 border-[#E2E8F0] hover:border-[#FFB347]/30 transition-all relative group">
                       ...
                       <div className="flex items-center justify-between mb-4">
                          ...
                          {pages.length > 1 && (
                             <button 
                                ...
                                className="w-8 h-8 rounded-xl flex items-center justify-center text-[#A0AEC0] hover:bg-[#FFE2E5] hover:text-[#FF4757] transition-all border-2 border-transparent hover:border-[#FF4757]/20 opacity-0 group-hover:opacity-100"
                             >
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {/* Text Input */}
                          <div>
                             <label className={labelClass}>Teks Bacaan</label>
                             <textarea 
                                placeholder="Tulis teks bacaan untuk halaman ini..." 
                                value={page.text} 
                                onChange={(e) => updatePageText(index, e.target.value)} 
                                className={`${inputClass} min-h-[180px] resize-y !bg-white`}
                                rows={6}
                             />
                          </div>
                          ...
                          {/* Image Upload */}
                          <div>
                             <label className={labelClass}>Gambar Ilustrasi</label>
                             {page.imagePreview ? (
                                <div className="w-full aspect-[4/3] rounded-2xl border-4 border-[#E2E8F0] overflow-hidden relative bg-white">
                                   ...
                                   <button 
                                      ...
                                      className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white/90 border-2 border-[#E2E8F0] flex items-center justify-center text-[#FF4757] hover:bg-[#FFE2E5] transition-all shadow-sm"
                                   >
                                      <span className="material-symbols-rounded text-lg">close</span>
                                   </button>
                                </div>
                             ) : (
                                <label className="w-full aspect-[4/3] border-4 border-dashed border-[#E2E8F0] rounded-2xl flex flex-col items-center justify-center gap-3 text-[#A0AEC0] hover:border-[#FFB347] hover:text-[#FFB347] hover:bg-[#FFF3E0]/30 transition-all cursor-pointer">
                                   <input type="file" accept="image/jpeg,image/png" onChange={(e) => handlePageImageUpload(index, e)} className="hidden" />
                                   <span className="material-symbols-rounded text-4xl">add_photo_alternate</span>
                                   <span className="text-xs font-black uppercase tracking-widest">Unggah Gambar</span>
                                </label>
                             )}
                          </div>
                       </div>
           ...
           <div className="bg-white rounded-[32px] p-8 md:p-10 border-4 border-[#E2E8F0] shadow-sm mb-8">
              <div className="flex items-center gap-3 mb-8 border-b-4 border-[#F8FAFC] pb-4">
                 <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-[#EDE9FE] border-2 border-[#C4B5FD] text-[#8B5CF6]">
                    <span className="material-symbols-rounded text-xl">picture_as_pdf</span>
                 </div>
                 <div>
                    <h2 className="text-xl font-black uppercase tracking-tight text-[#333333]">Unggah File Materi (PDF)</h2>
                    <p className="text-[10px] font-black text-[#A0AEC0] uppercase tracking-widest">File ini akan diproses menjadi animasi flipbook</p>
                 </div>
              </div>
              ...
              {pdfFileName ? (
                 <div className="flex items-center gap-4 bg-[#F8FAFC] rounded-2xl p-6 border-2 border-[#E2E8F0]">
                    <div className="w-14 h-14 rounded-2xl bg-[#EDE9FE] border-2 border-[#C4B5FD] flex items-center justify-center text-[#8B5CF6] shrink-0">
                       <span className="material-symbols-rounded text-3xl">picture_as_pdf</span>
                    </div>
                    <div className="flex-1 min-w-0">
                       <p className="text-base font-black text-[#333333] truncate">{pdfFileName}</p>
                       <p className="text-[10px] font-black text-[#A0AEC0] uppercase tracking-widest">{pdfFileSize}</p>
                    </div>
                    <button 
                       onClick={() => { setPdfFileName(null); setPdfFileSize(null); }}
                       className="w-10 h-10 rounded-xl flex items-center justify-center text-[#A0AEC0] hover:bg-[#FFE2E5] hover:text-[#FF4757] transition-all border-2 border-transparent hover:border-[#FF4757]/20"
                    >
                 ...
                 <button 
                    ...
                    className="w-full h-48 border-4 border-dashed border-[#E2E8F0] rounded-2xl flex flex-col items-center justify-center gap-3 text-[#A0AEC0] hover:border-[#8B5CF6] hover:text-[#8B5CF6] hover:bg-[#EDE9FE]/30 transition-all cursor-pointer group"
                 >
                    <span className="material-symbols-rounded text-5xl group-hover:scale-110 transition-transform">cloud_upload</span>
                    <span className="text-xs font-black uppercase tracking-widest">Klik untuk Unggah PDF</span>
                    <span className="text-[10px] font-bold">Format .PDF (Maks. 50 MB)</span>
                 </button>
              )}
           </div>
           ...
           <div className="flex flex-col sm:flex-row gap-4 justify-end sticky bottom-0 bg-[#F0F8FF]/90 backdrop-blur-xl py-6 -mx-10 px-10 border-t-4 border-[#E2E8F0] rounded-t-3xl z-40">
              <Link href="/admin/books" className="px-8 py-4 rounded-2xl text-sm font-black uppercase tracking-wider bg-white text-[#A0AEC0] border-4 border-[#E2E8F0] hover:border-[#cbd5e1] hover:text-[#666666] transition-all text-center shadow-sm">
                 Batal
              </Link>
              <button 
                 disabled={isSubmitting}
                 onClick={() => handleSubmit("draft")}
                 className={`px-8 py-4 rounded-2xl text-sm font-black uppercase tracking-wider bg-white text-[#8B5CF6] border-4 border-[#C4B5FD] hover:bg-[#EDE9FE] transition-all shadow-sm flex items-center justify-center gap-2 ${isSubmitting ? 'opacity-50 cursor-wait' : ''}`}
              >
                 <span className={`material-symbols-rounded text-lg ${isSubmitting && saveType === 'draft' ? 'animate-spin' : ''}`}>
                    {isSubmitting && saveType === 'draft' ? 'progress_activity' : 'save'}
                 </span>
                 {isSubmitting && saveType === 'draft' ? 'Menyimpan...' : 'Simpan Draft'}
              </button>
               <button 
                  disabled={isSubmitting}
                  onClick={() => handleSubmit("publish")}
                  className={`px-8 py-4 rounded-2xl text-sm font-black uppercase tracking-wider bg-[#FFB347] text-white border-4 border-[#E69A2E] shadow-[0_4px_0_#E69A2E] hover:shadow-[0_2px_0_#E69A2E] hover:translate-y-[2px] active:shadow-none active:translate-y-1 transition-all flex items-center justify-center gap-2 ${isSubmitting ? 'opacity-50 cursor-wait' : ''}`}
               >
                  <span className={`material-symbols-rounded text-lg ${isSubmitting && saveType === 'publish' ? 'animate-spin' : ''}`}>
                     {isSubmitting && saveType === 'publish' ? 'progress_activity' : 'publish'}
                  </span>
                  {isSubmitting && saveType === 'publish' ? 'Memproses...' : 'Simpan & Publikasikan'}
               </button>
            </div>
              ...
              <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
                 ...
                 <div className="bg-white rounded-[40px] p-10 border-4 border-[#E2E8F0] shadow-2xl relative z-10 max-w-sm w-full text-center animate-bounce-in">
                    <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 bg-[#F8FAFC] border-4 border-[#E2E8F0] relative overflow-hidden">
                       ...
                       />
                       <span className={`material-symbols-rounded text-3xl relative z-10 ${uploadProgress > 50 ? 'text-white' : 'text-[#FFB347]'} animate-pulse`}>
                          {saveType === 'publish' ? 'publish' : 'save'}
                       </span>
                    </div>
                    <h3 className="text-xl font-black text-[#333333] uppercase tracking-tighter mb-2">
                       {saveType === 'publish' ? 'Mempublikasikan...' : 'Menyimpan Draft...'}
                    </h3>
                    <p className="text-[#A0AEC0] font-bold text-xs leading-relaxed mb-6 h-8 flex items-center justify-center uppercase tracking-wide px-4">
                       {uploadStatus || "Memulai proses unggah..."}
                    </p>
           ...
           {showSuccessModal && (
             <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
                <div className="absolute inset-0 bg-[#333333]/40 backdrop-blur-sm animate-fade-in"></div>
                <div className="bg-white rounded-[40px] p-10 border-4 border-[#E2E8F0] shadow-2xl relative z-10 max-w-sm w-full text-center animate-bounce-in">
                   <div className={`w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6 border-8 ${saveType === 'publish' ? 'bg-emerald-100 border-emerald-50 text-emerald-500' : 'bg-[#FFF3E0] border-[#FFF3E0] text-[#FFB347]'}`}>
                      <span className="material-symbols-rounded text-5xl">
                         {saveType === 'publish' ? 'verified' : 'drafts'}
                      </span>
                   </div>
                   <h3 className="text-2xl font-black text-[#333333] uppercase tracking-tighter mb-2">
                      {saveType === 'publish' ? 'Berhasil Terbit!' : 'Draft Disimpan!'}
                   </h3>
                   <p className="text-[#666666] font-bold text-sm leading-relaxed mb-8">
                      {saveType === 'publish' 
                         ? 'Buku Anda sekarang sudah tersedia di perpustakaan digital.' 
                         : 'Buku telah disimpan dalam folder draft dan bisa dilanjutkan kapan saja.'}
                   </p>
                   ...
                   <p className="text-[10px] font-black text-[#A0AEC0] uppercase tracking-widest mt-4">
