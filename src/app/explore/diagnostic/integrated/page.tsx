...
export default function IntegratedDiagnosticPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F8FF] flex flex-col items-center font-body relative overflow-x-hidden text-[#333333]">
      {/* Background Decor */}
      <div className="cloud-blob w-[600px] h-[400px] -top-20 -right-20 animate-float-cloud"></div>
      <div className="cloud-blob w-[400px] h-[300px] bottom-10 left-10"></div>
      
      {/* Top Navbar */}
      <nav className="fixed top-0 left-0 right-0 z-[100] bg-white border-b-4 border-[#E2E8F0] shadow-sm">
        <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-3 flex items-center justify-between">
          <Link href="/explore/library" className="hover:scale-105 transition-transform flex items-center shrink-0">
             <Image src="https://i.ibb.co.com/cXwhYkn7/Desain-tanpa-judul-21.png" alt="Readify Logo" width={110} height={32} className="object-contain drop-shadow-sm" unoptimized />
          </Link>
          <div className="flex items-center gap-3">
             <div className="hidden md:flex items-center gap-2 mr-4">
                <Link href="/explore/library" className="px-4 py-2 rounded-xl bg-[#F0F8FF] text-[#5AAFD1] font-bold text-xs uppercase tracking-wide border-2 border-[#E2E8F0] hover:bg-white transition-all">Perpustakaan</Link>
             </div>
             <div className="flex items-center gap-3 bg-[#F8FAFC] px-4 py-1.5 rounded-full border-2 border-[#E2E8F0]">
                <div className="w-8 h-8 rounded-full border-2 border-[#FFB347] overflow-hidden"><img src={getAvatarUrl()} alt="Avatar" className="w-full h-full object-cover" /></div>
                <div className="hidden sm:block"><h4 className="text-[11px] font-black text-[#5AAFD1] uppercase truncate max-w-[80px]">{profile.name}</h4></div>
             </div>
          </div>
        </div>
      </nav>
